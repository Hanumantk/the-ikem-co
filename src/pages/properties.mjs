import { closing, listing, runhead } from "../helpers.mjs";
...
<section class="section" style="padding-block-start:0">
  <div data-tabs>
    ...
    <section class="panel" id="panel-active" role="tabpanel" aria-labelledby="tab-active">
      ...
      <div class="listings">
        ${listing({ lead: true, status: "Active", statusNote: "For lease", active: true, name: "6125 Bedford Avenue, Ladera Heights", href: "{{root}}contact.html?about=6125%20Bedford%20Avenue", price: "$7,210 per month", specs: "4 bed · 3 bath · 2,919 square feet · lot of about 6,740 square feet · built 1961", image: "home-bedford-day" })}
      </div>
      <div class="prose measure" style="margin-block-start:var(--space-2xl)">
    ...
    <section class="panel" id="panel-sold" role="tabpanel" aria-labelledby="tab-sold" hidden>
      ...
      <div class="listings listings--2">
        ${sold.map((s) => listing({ status: "Sold", name: s.name, href: "{{root}}contact.html", price: s.price, image: s.image, imagePos: s.imagePos })).join("\n        ")}
      </div>
    ...
    <section class="panel" id="panel-offmarket" role="tabpanel" aria-labelledby="tab-offmarket" hidden>
      ...
      ${runhead("By introduction", "Properties", 1)}
      <div class="prose measure dropcap">
        <p class="lede" style="margin-block-end:var(--space-lg);max-width:none">Some of the best houses in Los Angeles change hands without a sign, a listing, or a photograph online.</p>
...
${closing({ title: "Selling, or thinking about it?", line: "Ikem will tell you honestly whether a quiet sale or a public listing serves you better.", label: "Contact Ikem →" })}
`,
};
