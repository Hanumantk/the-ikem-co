...
- **Portrait photographs belong in portrait frames.** The full bleed bands are wide; a portrait file inside one can only show a fifth of its height on a large screen, which cuts the subject and leaves no drift. Use the landscape files (agave, window at dusk, dining table, concrete bench) for folds and closings, and the portrait files in the `px--portrait` frames on the About page.
